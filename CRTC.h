...
struct str_heure
{
	...
	CRTC::MODE_HEURE mode; ///< mode des heures
...
class CRTC : public CI2C
{
	public:
		...
		class ErreurModeHeure : public exception {
			...
			public:
				ErreurWrite(MODE_HEURE  mode):m_mode(mode) {};
				virtual const char* what() const noexcept {
					string temp = "Le mode des heures ne correspond pas : mode ";
					if( m_mode == MODE_HEURE::12H )
						temp += "12H"
					else
						temp += "24H"
					return temp.c_str();
	...
	public:
		 ...
		 */
		int getRTCHeures(PERIODE_HORAIRE &pHoraire) const;
